...
> **Google Ads (conta `AW-18257647048`):** o `gtag.js` é carregado no boot (`loadGoogleAds` em `track.js`) e dispara duas conversões manuais: **Cadastro Cauril** (`X7IjCKnd2MIcEMir9oFE`) no `signup_success` e **Assinatura Cauril** (`h1A0CKzd2MIcEMir9oFE`, com `value`/`currency`/`transaction_id`) no `purchase_success`. Rótulos vêm do painel Google Ads (Metas → Conversões). As duas estão marcadas como "Principal" e precisam ser aplicadas como **meta da campanha** "Cauril - Análise Carteira [Search]" pro algoritmo otimizar por elas. Sem `AW-*` configurado, vira no-op.
