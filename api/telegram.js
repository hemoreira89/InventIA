// Webhook do bot Telegram do InvestIA.
...
async function chamarGemini(apiKey, contexto, pergunta) {
  ...
  const prompt = `Você é o assistente financeiro do InvestIA, especializado em carteiras da B3.
...
async function handleVinculo(chatId, code, serviceKey, botToken) {
  ...
  if (!link) {
    await enviarMensagem(chatId, "❌ Código inválido. Gere um novo código no app InvestIA.", botToken);
    return;
  }
  if (link.used) {
  ...
  if (new Date(link.expires_at) < new Date()) {
    await enviarMensagem(chatId, "❌ Código expirado. Gere um novo código no app InvestIA.", botToken);
    return;
  }
